...
public class HtmlExportTestsBuilder extends ExportTestBuilder {
    ...
    private static String printStringCombinationHtml(List<Combination> combinations) {
        ...
        while (var2.hasNext()) {
            ...
            str.append("<br>Combination:<br>");

            Map.Entry pair;
            for (Iterator it = combination.getValues().entrySet().iterator(); it.hasNext(); str.append(" ").append(pair.getKey()).append(" - ").append(pair.getValue()).append("<br>")) {
